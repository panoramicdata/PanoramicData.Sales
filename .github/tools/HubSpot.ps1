...
function Get-HubSpotRecentManualDeals {
    ...
    # Get recent deals
    $result = Search-HubSpotDeals -Query "*" -Limit 200 -ExcludeAutoGenerated
    
    # Filter by date range
    $cutoffDate = (Get-Date).AddDays(-$DaysBack)
    $recentDeals = $result.results | Where-Object { 
        [datetime]$_.createdAt -gt $cutoffDate 
    } | Sort-Object { [datetime]$_.createdAt } -Descending | Select-Object -First $Limit
    
    return @{
        "total" = $recentDeals.Count
        "results" = $recentDeals
    }
}

# Function to get deals created by a specific user or on a specific date
function Get-HubSpotDealsByDateOrUser {
    ...
    $result = Search-HubSpotDeals -Query "*" -Limit 200 -ExcludeAutoGenerated:$ExcludeAutoGenerated
    
    $filteredDeals = $result.results
    
    if ($Date) {
        Write-Host "Filtering deals created on: $Date"
        $filteredDeals = $filteredDeals | Where-Object { 
            ([datetime]$_.createdAt).ToString("yyyy-MM-dd") -eq $Date 
        }
    }
    ...
    if ($UserId) {
        ...
        $filteredDeals = $filteredDeals | Where-Object { 
            $_.properties.hs_created_by_user_id -eq $UserId 
        }
    }
    
    $filteredDeals = $filteredDeals | Sort-Object { [datetime]$_.createdAt } -Descending | Select-Object -First $Limit
