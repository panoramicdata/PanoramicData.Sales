...
switch ($Action.ToLower()) {
    ...
    "closing" {
        # Get deals closest to closing - formatted output with NO semicolons needed in caller
        $dealsResult = Search-HubSpotDeals -Query "*" -Properties @("dealname","amount","closedate","dealstage") -Limit 200 -ExcludeAutoGenerated $true
        # Include all open stages (not closed), prioritizing those nearest to close
        $closingDeals = $dealsResult.results | Where-Object { 
            $_.properties.dealstage -notmatch 'closed' -and $_.properties.closedate 
        } | Sort-Object { [DateTime]$_.properties.closedate } | Select-Object -First 10
        
        if ($closingDeals) {
            Write-Host "`nTop 10 deals closest to closing date:" -ForegroundColor Cyan
            $closingDeals | ForEach-Object {
                [PSCustomObject]@{
                    Name = $_.properties.dealname
                    Amount = if($_.properties.amount){"£$($_.properties.amount)"} else {"£0"}
                    CloseDate = $_.properties.closedate
                    Stage = $_.properties.dealstage
                }
            } | Format-Table -AutoSize
        } else {
            Write-Host "No open deals found with close dates" -ForegroundColor Yellow
        }
    }
    
    "stale" {
        # Find long-standing deals that should probably be closed lost
        $dealsResult = Search-HubSpotDeals -Query "*" -Properties @("dealname","amount","closedate","dealstage","hs_lastmodifieddate","createdate") -Limit 200 -ExcludeAutoGenerated $true
        $currentDate = Get-Date
        
        # Filter for deals that are:
        # 1. Not already closed
        # 2. Have close dates more than 6 months overdue OR created more than 1 year ago
        # 3. Haven't been modified in the last 3 months
        $staleDeals = $dealsResult.results | Where-Object { 
            $_.properties.dealstage -notmatch 'closed' -and (
                # Overdue by more than 6 months
                ($_.properties.closedate -and ([DateTime]$_.properties.closedate).AddMonths(6) -lt $currentDate) -or
                # Created more than 1 year ago
                ([DateTime]$_.properties.createdate).AddYears(1) -lt $currentDate
            ) -and
            # Not modified in last 3 months
            ([DateTime]$_.properties.hs_lastmodifieddate).AddMonths(3) -lt $currentDate
        } | Sort-Object { [DateTime]$_.properties.closedate } 
        
        if ($staleDeals) {
            Write-Host "`nLong-standing deals that should probably be closed lost:" -ForegroundColor Red
            Write-Host "Criteria: Overdue 6+ months OR created 1+ year ago AND not modified in 3+ months`n" -ForegroundColor Yellow
            
            $staleDeals | ForEach-Object {
                $overdueDays = if ($_.properties.closedate) { 
                    [Math]::Round(($currentDate - [DateTime]$_.properties.closedate).TotalDays) 
                } else { "No close date" }
                
                $daysSinceModified = [Math]::Round(($currentDate - [DateTime]$_.properties.hs_lastmodifieddate).TotalDays)
                ...
                [PSCustomObject]@{
                    ...
                    LastModified = $_.properties.hs_lastmodifieddate
                    DaysSinceModified = $daysSinceModified
                    CloseDate = if($_.properties.closedate) { $_.properties.closedate } else { "None" }
                }
            } | Format-Table -AutoSize
            
            $totalValue = ($staleDeals | Where-Object { $_.properties.amount } | ForEach-Object { [decimal]$_.properties.amount } | Measure-Object -Sum).Sum
            Write-Host "Total value of stale deals: £$totalValue" -ForegroundColor Cyan
            Write-Host "Number of stale deals: $($staleDeals.Count)" -ForegroundColor Cyan
        } else {
            Write-Host "No stale deals found matching criteria" -ForegroundColor Green
        }
    }
    
    "overdue" {
        # Find deals that are overdue by more than 3 months
        $dealsResult = Search-HubSpotDeals -Query "*" -Properties @("dealname","amount","closedate","dealstage","hs_lastmodifieddate") -Limit 200 -ExcludeAutoGenerated $true
        $currentDate = Get-Date
        $threeMonthsAgo = $currentDate.AddMonths(-3)
        
        $overdueDeals = $dealsResult.results | Where-Object { 
            $_.properties.dealstage -notmatch 'closed' -and 
            $_.properties.closedate -and 
            ([DateTime]$_.properties.closedate) -lt $threeMonthsAgo
        } | Sort-Object { [DateTime]$_.properties.closedate } 
        
        if ($overdueDeals) {
            Write-Host "`nDeals overdue by more than 3 months:" -ForegroundColor Yellow
            
            $overdueDeals | ForEach-Object {
                $overdueDays = [Math]::Round(($currentDate - [DateTime]$_.properties.closedate).TotalDays)
                $daysSinceModified = [Math]::Round(($currentDate - [DateTime]$_.properties.hs_lastmodifieddate).TotalDays)
            ...
            $totalValue = ($overdueDeals | Where-Object { $_.properties.amount } | ForEach-Object { [decimal]$_.properties.amount } | Measure-Object -Sum).Sum
            Write-Host "Total value of overdue deals: £$totalValue" -ForegroundColor Cyan
            Write-Host "Number of overdue deals: $($overdueDeals.Count)" -ForegroundColor Cyan
        } else {
            Write-Host "No deals found overdue by more than 3 months" -ForegroundColor Green
        }
    }
...
Examples:
  ...
  # Get deals closest to closing (formatted output)
  .\HubSpot.ps1 -Action closing -ObjectType deals
