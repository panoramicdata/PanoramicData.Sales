...
switch ($Action.ToLower()) {
    ...
    "closing" {
        ...
        $deals = Search-HubSpotDeals -Query "*" -Properties @("dealname","amount","closedate","dealstage") -Limit 200 -ExcludeAutoGenerated $true
        $closingStages = @("contractsent","decisionmakerboughtin","presentationscheduled")
        $closingDeals = $deals | Where-Object { 
            $_.properties.dealstage -in $closingStages -and $_.properties.closedate 
        } | Sort-Object { [DateTime]$_.properties.closedate } | Select-Object -First 10
        
        $closingDeals | ForEach-Object {
            [PSCustomObject]@{
                Name = $_.properties.dealname
                Amount = if($_.properties.amount){"£$($_.properties.amount)"} else {"N/A"}
